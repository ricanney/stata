...
qui { // module 3 - convert summaryqc to sumstats
	...
	noi di as text"#########################################################################"
	local already_created ${init_root}/summaryqc2sumstats/``function'_bim_short'/``function'_bim_short'-summaryqc2sumstats-hw3.sumstats
...
qui { // module 4 - define h2
	noi di as text"#########################################################################"
	noi di as text"# SECTION - 3: apply `ldsc'"
	noi di as text"#########################################################################"
	local `function'_sumstat ${init_root}/summaryqc2sumstats/``function'_bim_short'/``function'_bim_short'-summaryqc2sumstats-hw3.sumstats
	noi di as text"# > ..... project name "as result"``function'_bim_short'"
	noi di as text"#########################################################################"
	qui { // observed scale
		...
		keep v1
		noi di as text"# >  h2 observed-scale "as result`"`: display %05.3f ``function'_o_h2'' (`: display %05.3f ``function'_o_h2_se'')"'
		noi di as text"# > ........ lambda GC "as result`"`: display %05.3f ``function'_o_lambda''"'
		noi di as text"# > ....... mean chi^2 "as result`"`: display %05.3f ``function'_o_chi''"'
		noi di as text"# > ........ intercept "as result`"`: display %05.3f ``function'_o_intercept'' (`: display %05.3f ``function'_o_intercept_se'')"'
		noi di as text"#########################################################################"
		}
	qui { // liability scale
		clear
		...
		if pop == "" {
			...
			gen pop = "`pop_prev'"
			gen samp = "`samp_prev'"
			for var pop samp: destring X, replace force
			noi di as text"# > h2 liability-scale "as result`"`: display %05.3f ``function'_l_h2'' (`: display %05.3f ``function'_l_h2_se'')"'
			noi di as text"# > ........ lambda GC "as result`"`: display %05.3f ``function'_l_lambda''"'
			noi di as text"# > ....... mean chi^2 "as result`"`: display %05.3f ``function'_l_chi''"'
			noi di as text"# > ........ intercept "as result`"`: display %05.3f ``function'_l_intercept'' (`: display %05.3f ``function'_l_intercept_se'')"'
			if pop == . {
				noi di as text"# >  based on pop-prev "as result"NaN"
				noi di as text"# > based on samp-prev "as result"NaN"
				}
			else {
				noi di as text"# >  based on pop-prev "as result`"`: display %05.3f `pop_prev''"'
				noi di as text"# > based on samp-prev "as result`"`: display %05.3f `samp_prev''"'
				}
			noi di as text"#########################################################################"
			noi di as text""
			}
...
qui { // module 4 - write log file
	...
	file write myfile `"# >  h2 observed-scale `: display %05.3f ``function'_o_h2'' (`: display %05.3f ``function'_o_h2_se'')"' _n
	clear
	set obs 1
	gen pop = "`pop_prev'"
	if pop == "" {
		...
		file write myfile `"# > h2 liability-scale `: display %05.3f ``function'_l_h2'' (`: display %05.3f ``function'_l_h2_se'')"' _n
		file write myfile `"# > ......... pop_prev `: display %05.3f `pop_prev''"' _n
		file write myfile `"# > ........ samp_prev `: display %05.3f `samp_prev''"' _n
		}
	file write myfile `"# > ........ lambda GC `: display %05.3f ``function'_l_lambda''"' _n
	file write myfile `"# > ....... mean chi^2 `: display %05.3f ``function'_l_chi''"' _n
	file write myfile `"# > ........ intercept `: display %05.3f ``function'_l_intercept'' (`: display %05.3f ``function'_l_intercept_se'')"'	_n
