...
if !_rc {
	noi checkfile, file(`main_output')
	}
else {	
	...
	qui { // module 6 - create bim.dta files
		noi di as text"#########################################################################"
		noi di as text"# SECTION - 6: create bim.dta files"
		noi di as text"#########################################################################"	
		cd ${init_root}	
		!mkdir bim2dta
		cd bim2dta
		!mkdir all-hrc-1.1-chrall-mac5
		cd ${init_root}/`function'/HRC.r1-1.GRCh37.wgs.mac5
		use HRC.r1-1.GRCh37.wgs.mac5.sites.core.chrall.dta, clear
		keep  chr snp bp a1 a2 gt loc_name
		order chr snp bp a1 a2 gt loc_name
		save ${init_root}/bim2dta/all-hrc-1.1-chrall-mac5/all-hrc-1.1-chrall-mac5_bim.dta, replace
		noi checkfile, file(`main_output')
